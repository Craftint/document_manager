        ...
        for i in doc.related_documents_table:
            ...
            if datetime.now().date() >= creation_deadline:
                ...
                if doc.doctype == 'Employee':
                    doc_expiry.user = doc.user_id
                    doc_expiry.whatsapp_number = doc.cell_number
                
                doc_assignment = frappe.get_doc('Document Assignment', "Employee")
                doc_expiry.recipient = []
                if doc_assignment.dt == 'Employee':
